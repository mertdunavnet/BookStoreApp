...
namespace BookStoreApp.API.Controllers
{
    ...
    public class BooksController : ControllerBase
    {
        ...
        // GET: api/Books
        [HttpGet]
        public async Task<ActionResult<List<ResultBookDto>>> GetBooks()
        {
            var books = await _context.Books
                .Include(q => q.Author)
                .ProjectTo<ResultBookDto>(mapper.ConfigurationProvider)
                .ToListAsync();
            //var response = mapper.Map<List<ResultBookDto>>(books);
            return Ok(books);
        }

        // GET: api/Books/5
        [HttpGet("{id}")]
        public async Task<ActionResult<ResultBookDetailsDto>> GetBook(int id)
        {
            var book = await _context.Books
                .Include(q => q.Author)
                .ProjectTo<ResultBookDetailsDto>(mapper.ConfigurationProvider)
